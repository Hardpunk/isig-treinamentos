		...
		Bonsai.prototype = {
			...
			setExpanded : function(listItem, expanded) {
				...
				if (expanded) {
					...
					$li = $($li).addClass('collapsed').removeClass('expanded');
			...
			insertInput : function(listItem) {
				...
				var checkbox = $('<input type="' + type + '" name="' + this.inputNameFor(listItem) + '" id="' + id + '" /> ');
				...
				children.detach();
				listItem.append(checkbox).append($('<label for="' + id + '">').append(text.length > 0 ? text : children.first())).append(text.length > 0 ? children : children.slice(1));
			},
			...
			handleDuplicateCheckboxes : function() {
				...
				self.el.on('change', 'input[type=checkbox]', function(ev) {
					...
					var selector = 'input[type=checkbox]' + '[value="' + checkbox.val() + '"]' + (checkbox.attr('name') ? '[name="' + checkbox.attr('name') + '"]' : '') + (checkbox.prop('checked') ? ':not(:checked)' : ':checked');
